 ...
 *
 * REGEN-ON-CORRECTION: when family proofread a page in the app, update that page's
 * Chinese in data/transcription.js, then regenerate ITS English (prose → full
 * translation; register → 1–2 line summary) into data/transcription_en.json, then
 * run this script. See HANDOFF.md → "Page transcriptions & English reading aid".
 */
const fs = require("fs");
...
const STYLE = `  body { font-family: "PingFang SC", "Heiti SC", sans-serif; line-height: 1.8; padding: 20px; background: #fff; color: #333; max-width: 820px; margin: 0 auto; }
  h1 { text-align: center; color: #2c3e50; border-bottom: 2px solid #eee; padding-bottom: 10px; }
  h2 { color: #d35400; margin-top: 34px; border-left: 4px solid #d35400; padding-left: 10px; }
  ul { list-style-type: square; padding-left: 20px; margin-bottom: 10px; }
...
const NOTE =
  ...
  '此為手抄本《江氏族譜》的逐頁釋文，由機器視覺結合人工校讀而成。原稿為行草手寫、年代久遠，' +
  '部分頁面褪色嚴重；括號 <code>〔 〕</code> 內為字跡難辨之推測或缺失標記。<br>' +
  '英文為輔助閱讀之詮釋，非權威翻譯：散文頁（序文、源流、遷徙記）為全文翻譯，' +
  '名錄頁僅附簡短摘要。家人於校讀器更正某頁中文後，該頁英文亦應隨之重新生成；' +
  '在此之前，英文係依原始機器釋文，可能與已更正之內容不符。釋文與翻譯僅供參考，歡迎家人對照原稿協助校正。<br>' +
  '<em>English below each page is an interpretive reading aid, not an authoritative translation — ' +
  'prose pages (prefaces, origin and migration accounts) are translated in full; ' +
  'register / name-list pages carry a short summary only. ' +
  'When a page’s Chinese is corrected in the proofreader, its English should be regenerated; ' +
  'until then the English reflects the original machine transcription and may not match corrected pages.</em>';

const FOOT =
  '<em>(以上釋文與英文詮釋基於手抄本掃描，以人工智慧輔以校讀。因屬行草手寫，' +
...
const n1 = build("book_pt1", "pt1", "江氏族譜 — 手抄本 上冊 釋文 (Part 1)", "data/Kong_Family_book_pt1_Transcription.html");
const n2 = build("book_pt2", "pt2", "江氏族譜 — 手抄本 下冊 釋文 (Part 2)", "data/Kong_Family_book_pt2_Transcription.html");
console.log(`built pt1 (${n1} pp) + pt2 (${n2} pp) from transcription.js + transcription_en.json`);
